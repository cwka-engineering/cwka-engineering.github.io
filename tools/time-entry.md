...
- **Direct entries on the ENG bucket (`####.ENG`)** — every ENG-bucket operation (**Design Engineering**, **Fabrication Engineering**, **Lead Coordination**, **Project Meeting**, and **BIM Coordination**): A labor note is **required**. These operations carry the least job-specific context of any direct work — without a note there's no way to reconstruct what was actually done, which matters most when back-calculating engineering costs on a change order.
- **All other direct entries** (E-job, GS, and PE work): A labor note is **strongly preferred**, not required. A note is always worth adding here too — change-order back-calculation and cost reconciliation depend on it, and a blank note is a lost data point even where it isn't mandatory.
