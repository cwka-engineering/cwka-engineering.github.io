...
- **Direct entries on the ENG bucket (`####.ENG`)** — **Design Engineering**, **Fabrication Engineering**, **Lead Coordination**, and **BIM Coordination**: A labor note is **required**. These four operations carry the least job-specific context of any direct work — without a note there's no way to reconstruct what was actually done, which matters most when back-calculating engineering costs on a change order. **Project Meeting** on the bucket is not included in this requirement; see below.
- **All other direct entries** (E-job, GS, and PE work, plus Project Meeting on the bucket): A labor note is **strongly preferred**, not required. A note is always worth adding here too — change-order back-calculation and cost reconciliation depend on it, and a blank note is a lost data point even where it isn't mandatory.
...
*Labor note: **required** — see [Labor note requirements](#labor-note-requirements).*
...
*Labor note: strongly preferred, not required — the one ENG-bucket operation not covered by the required-note rule above.*
