...
- **EA (Engineering Assistant)**: Legacy indirect-code label; not a current hiring path. Most of this scope (Epicor materials database, job creation, scheduling) is now owned by the **Master Scheduler**, an Operations-department role — see [Master Scheduler Workflow](/workflows/master-scheduler.html).
